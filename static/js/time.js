$(function() {
  ...
  $("#reportrange").on("apply.daterangepicker", function(ev, picker) {
    ...
    $.ajax({
     ...
     success: function(data) {
       ...
       var capitalizedTableName = selectedTable.charAt(0).toUpperCase() + selectedTable.slice(1);
        $('#selectedTableName').text(capitalizedTableName).css('background-color', '#fffff;');
}
    });
  });
 ...
 $('#averageodo').html(data);
       $('#averageodo').append(data.htmlresponse);
              ...
              var capitalizedTableName = selectedTable.charAt(0).toUpperCase() + selectedTable.slice(1);
        $('#selectedTableName').text(capitalizedTableName).css({
              'background-color': '#95a2a3',
                    'text-align': 'center',
                        "border": "1px solid #825e7d",
